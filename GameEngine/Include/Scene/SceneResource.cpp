...
bool CSceneResource::LoadMesh(std::string& OutName, Mesh_Type Type, const TCHAR* FileName, const std::string& PathName)
{
	...
	m_mapMesh.insert(std::make_pair(OutName, CResourceManager::GetInst()->FindMesh(OutName)));
	return true;
}
...
bool CSceneResource::LoadTextureFullPath(std::string& OutName, const TCHAR* FullPath)
{
	if (!CResourceManager::GetInst()->LoadTextureFullPath(OutName, FullPath))
		return false;

	m_mapTexture.insert(std::make_pair(OutName, CResourceManager::GetInst()->FindTexture(OutName)));
...
bool CSceneResource::LoadAnimationSequenceFullPathMultibyteSetOriginFileName(bool Loop, std::string& Name, 
	const char* FullPath)
{
	if (FindAnimationSequence(Name))
		return true;

	if (!CResourceManager::GetInst()->LoadAnimationSequenceFullPathMultibyteSetOriginFileName(Loop, Name,
		FullPath))
		return false;


	CAnimationSequence2D* Sequence2D = CResourceManager::GetInst()->FindAnimationSequence2D(Name);
